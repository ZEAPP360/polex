...
class User
{
    ...
    if (isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
      ...
      if ($get_user->num_rows > 0) {
        ...
        if ($system['monetization_enabled']) {
          $this->_data['can_monetize_content'] = $this->check_module_permission($system['monetization_permission']);
        }
        ...
        if ($system['gifts_enabled']) {
          $this->_data['can_send_gifts'] = $this->check_module_permission($system['gifts_permission']);
        }
        ...
        if ($system['colored_posts_enabled']) {
          $this->_data['can_add_colored_posts'] = $this->check_module_permission($system['colored_posts_permission']);
        }
        /* check activity posts permission */
        if ($system['activity_posts_enabled']) {
          $this->_data['can_add_activity_posts'] = $this->check_module_permission($system['activity_posts_permission']);
        }
        /* check polls posts permission */
        if ($system['polls_enabled']) {
          $this->_data['can_add_polls_posts'] = $this->check_module_permission($system['polls_posts_permission']);
        }
        /* check geolocation posts permission */
        if ($system['geolocation_enabled']) {
          $this->_data['can_add_geolocation_posts'] = $this->check_module_permission($system['geolocation_posts_permission']);
        }
        /* check gif posts permission */
        if ($system['gif_enabled']) {
          $this->_data['can_add_gif_posts'] = $this->check_module_permission($system['gif_posts_permission']);
        }
        /* check anonymous posts permission */
        if ($system['anonymous_mode']) {
          $this->_data['can_add_anonymous_posts'] = $this->check_module_permission($system['anonymous_posts_permission']);
        }
        ...
        if ($system['videos_enabled']) {
          $this->_data['can_upload_videos'] = $this->check_module_permission($system['videos_upload_permission']);
        }
        /* check audios upload permission */
        if ($system['audio_enabled']) {
          $this->_data['can_upload_audios'] = $this->check_module_permission($system['audios_upload_permission']);
        }
        /* check files upload permission */
        if ($system['file_enabled']) {
          $this->_data['can_upload_files'] = $this->check_module_permission($system['files_upload_permission']);
        }
  ...
  public function get_countries()
  {
    global $db, $system;
    $countries = [];
    $get_countries = $db->query("SELECT * FROM system_countries WHERE enabled = '1' ORDER BY country_id ASC") or _error("SQL_ERROR_THROWEN");
   ...
   * @return array
   */
  public function get_subscribers($node_id, $node_type = 'user', $offset = 0, $get_all = false)
  {
    global $db, $system;
    $subscribers = [];
    $offset *= $system['max_results_even'];
   ...
   * @return integer
   */
  public function get_subscribers_count($node_id = null, $node_type = 'user')
  {
    global $db;
   ...
   * @return boolean
   */
  public function is_subscribed($node_id, $node_type = 'user')
  {
    global $db;
    if ($this->_logged_in) {
      /* check if the viewer is subscribed to the target and insertdate less than a month */
      $check = $db->query(sprintf("SELECT COUNT(*) as count FROM subscribers WHERE node_type = %s AND node_id = %s AND user_id = %s AND time > DATE_SUB(NOW(), INTERVAL 1 MONTH)", secure($node_type), secure($node_id, 'int'), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
      if ($check->fetch_assoc()['count'] > 0) {
        return true;
      }
    }
   ...
   * @return void
   */
  public function subscribe($node_id, $node_type, $user_id = null)
  {
    global $db, $date;
    /* check user */
    $user_id = ($user_id) ? $user_id : $this->_data['user_id'];
    /* check of the node is valid */
    switch ($node_type) {
      case 'user':
        ...
        $content_price = $node['user_monetization_price'];
        $notification_action = 'subscribe_user';
      ...
      case 'page':
        ...
        $content_price = $node['page_monetization_price'];
      ...
      case 'group':
        ...
        $content_price = $node['group_monetization_price'];
    ...
    /* check if the viewer already subscribe the target */
    $check = $db->query(sprintf("SELECT COUNT(*) as count FROM subscribers WHERE user_id = %s AND node_id = %s AND node_type = %s", secure($user_id, 'int'),  secure($node_id, 'int'), secure($node_type))) or _error("SQL_ERROR_THROWEN");
    /* if yes -> return error */
    if ($check->fetch_assoc()['count'] > 0) {
      _error(400);
    };
    /* add as subscriber */
    $db->query(sprintf("INSERT INTO subscribers (user_id, node_id, node_type, time) VALUES (%s, %s, %s, %s)", secure($user_id, 'int'),  secure($node_id, 'int'), secure($node_type), secure($date))) or _error("SQL_ERROR_THROWEN");
    /* prepare commission */
    ...
    $db->query(sprintf("UPDATE users SET user_monetization_balance = user_monetization_balance + %s WHERE user_id = %s", secure($content_price, 'int'), secure($content_creator_id, 'int'))) or _error("SQL_ERROR_THROWEN");
   ...
   * @return void
   */
  public function unsubscribe($node_id, $node_type)
  {
    global $db;
    /* check if the viewer already subscribe the target */
    $check = $db->query(sprintf("SELECT COUNT(*) as count FROM subscribers WHERE user_id = %s AND node_id = %s AND node_type = %s", secure($this->_data['user_id'], 'int'),  secure($node_id, 'int'), secure($node_type))) or _error("SQL_ERROR_THROWEN");
    /* if no -> return */
    if ($check->fetch_assoc()['count'] == 0) return;
    /* delete from viewer's subscribers */
    $db->query(sprintf("DELETE FROM subscribers WHERE user_id = %s AND node_id = %s AND node_type = %s", secure($this->_data['user_id'], 'int'), secure($node_id, 'int'), secure($node_type))) or _error("SQL_ERROR_THROWEN");
    /* delete notification */
    $this->delete_notification($node_id, 'subscribe');
  }
  ...
  public function post_notification($args = [])
  {
    ...
    if ($system['email_notifications']) {
      ...
      if ($notification['message']) {
        $subject = __("New notification from") . " " . __($system['system_title']);
   ...
   * @return integer
   */
  public function create_live_post($agora_channel_name, $video_thumbnail, $node = null, $node_id = null)
  {
    global $db, $system, $date;
    ...
    /* insert post */
    $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, in_group, group_id, group_approved, in_event, event_id, event_approved, privacy, time) VALUES (%s, %s, 'live', %s, %s, %s, %s, %s, %s, %s, %s)", secure($post['user_id'], 'int'), secure($post['user_type']), secure($post['in_group']), secure($post['group_id'], 'int'), secure($post['group_approved']), secure($post['in_event']), secure($post['event_id'], 'int'), secure($post['event_approved']), secure($post['privacy']), secure($date))) or _error("SQL_ERROR_THROWEN");
  ...
  public function scraper($url)
  {
    ...
    $embed = Embed\Embed::create($url, null);
    if ($embed) {
      $return = [];
      ...
      if ($embed->type == "link" || ($embed->type == "rich" && !in_array($embed->providerName, ["Twitter", "SoundCloud", "Mixcloud"]))) {
        ...
        if ($this->_data['user_group'] == 3 && $post['author_id'] != $this->_data['user_id']) {
          if ($post['in_group'] && $post['group_monetization_enabled']) {
            ...
            if (!$this->is_subscribed($post['group_id'], 'group')) {
              ...
              $post['needs_subscription_price'] = $post['group_monetization_price'];
            }
            ...
            if (!$this->is_subscribed($post['page_id'], 'page')) {
              ...
              $post['needs_subscription_price'] = $post['page_monetization_price'];
            }
          } elseif ($post['user_type'] == "user" && $post['user_monetization_enabled']) {
            /* user */
            /* check if the viewer subscribed to this user */
            if (!$this->is_subscribed($post['author_id'], 'user')) {
              $post['needs_subscription'] = true;
              $post['needs_subscription_type'] = 'user';
              $post['needs_subscription_id'] = $post['author_id'];
              $post['needs_subscription_price'] = $post['user_monetization_price'];
            }
          }
        }
      } else {
  ...
  public function edit_page($page_id, $edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'monetization':
        ...
        if ($args['page_monetization_enabled'] && (!is_numeric($args['page_monetization_price']) || $args['page_monetization_price'] <= 0)) {
          throw new Exception(__("You have entered an invalid price"));
        }
        /* update page */
        $db->query(sprintf("UPDATE pages SET page_monetization_enabled = %s, page_monetization_price = %s WHERE page_id = %s", secure($args['page_monetization_enabled']), secure($args['page_monetization_price'], 'float'), secure($page_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
    }
  ...
  public function edit_group($group_id, $edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'monetization':
        ...
        /* check if price is valid */
        if ($args['group_monetization_enabled'] && (!is_numeric($args['group_monetization_price']) || $args['group_monetization_price'] <= 0)) {
          throw new Exception(__("You have entered an invalid price"));
        }
        /* update group */
        $db->query(sprintf("UPDATE `groups` SET group_monetization_enabled = %s, group_monetization_price = %s WHERE group_id = %s", secure($args['group_monetization_enabled']), secure($args['group_monetization_price'], 'float'), secure($group_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
    }
  ...
  public function get_articles($args = [])
  {
    ...
    $get_posts = $db->query(sprintf("SELECT posts.post_id FROM posts INNER JOIN posts_articles ON posts.post_id = posts_articles.post_id LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user' LEFT JOIN pages ON posts.user_id = users.user_id AND posts.user_type = 'page' WHERE posts.in_group = '0' AND posts.in_event = '0' AND posts.post_type = 'article' " . $where_query . " " . $order_query . " LIMIT %s, %s", secure($offset, 'int', false), secure($results, 'int', false))) or _error("SQL_ERROR_THROWEN");
    if ($get_posts->num_rows > 0) {
      while ($post = $get_posts->fetch_assoc()) {
        $post = $this->get_post($post['post_id']);
        if ($post) {
  ...
  public function create_campaign($args = [])
  {
    ...
    if ($args['campaign_budget'] < max($system['ads_cost_click'], $system['ads_cost_view'])) {
      throw new Exception(__("The minimum budget must be") . " <strong>" . print_money(max($system['ads_cost_click'], $system['ads_cost_view'])) . "</strong> ");
    }
    if ($this->_data['user_wallet_balance'] < $args['campaign_budget']) {
      throw new Exception(__("Your current wallet credit is") . " <strong>" . print_money($this->_data['user_wallet_balance']) . "</strong> " . __("You need to") . " <a href='" . $system['system_url'] . "/wallet'>" . __("Replenish wallet credit") . "</a>");
    }
  ...
  public function edit_campaign($campaign_id, $args)
  {
    ...
    if ($remaining > $campaign['user_wallet_balance']) {
      throw new Exception(__("The remaining spend is larger than current wallet credit") . " <strong>" . print_money($campaign['user_wallet_balance']) . "</strong> " . __("You need to") . " " . __("Replenish wallet credit"));
    }
    /* validate campaign bidding */
    if (!in_array($args['campaign_bidding'], array('click', 'view'))) {
      throw new Exception(__("You have to select a valid bidding"));
    ...
    if ($args['campaign_bidding'] == "click") {
      if ($remaining < $system['ads_cost_click']) {
        throw new Exception(__("The cost per click is larger than your campaign remaining spend") . " <strong>" . print_money($remaining) . "</strong> " . __("increase the campaign budget to resume the campaign"));
      }
    }
    if ($args['campaign_bidding'] == "view") {
      if ($remaining < $system['ads_cost_view']) {
        throw new Exception(__("The cost per view is larger than your campaign remaining spend") . " <strong>" . print_money($remaining) . "</strong> " . __("increase the campaign budget to resume the campaign"));
      }
    }
    /* validate audience countries */
    $args['audience_countries'] = (isset($args['audience_countries']) && is_array($args['audience_countries'])) ? $this->spread_ids($args['audience_countries']) : "";
  ...
  public function wallet_withdraw_points($amount)
  {
    ...
    if ($points_balance < $amount) {
      throw new Exception(__("The amount is larger than your current points balance") . " <strong>" . print_money($points_balance) . "</strong>");
    }
  ...
  public function wallet_withdraw_monetization($amount)
  {
    ...
    if ($this->_data['user_monetization_balance'] < $amount) {
      throw new Exception(__("The amount is larger than your current monetization balance") . " <strong>" . print_money($this->_data['user_monetization_balance']) . "</strong>");
    }
   ...
   * @return void
   */
  public function wallet_monetization_payment($node_id, $node_type)
  {
    global $db, $system;
    /* check if wallet enabled */
    if (!$system['wallet_enabled']) {
    ...
    if ($this->is_subscribed($node_id, $node_type)) {
      modal("SUCCESS", __("Subscribed"), __("You already subscribed to this") . " " . __($node_type));
    }
    /* check viewer balance */
    if ($this->_data['user_wallet_balance'] < $price) {
      modal("ERROR", __("Sorry"), __("There is no enough credit in your wallet to buy this") . ", " . __("Recharge your wallet to continue"));
    }
    /* decrease viewer user wallet balance */
    $db->query(sprintf('UPDATE users SET user_wallet_balance = IF(user_wallet_balance-%1$s<=0,0,user_wallet_balance-%1$s) WHERE user_id = %2$s', secure($price), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
    /* log this transaction */
    $this->wallet_set_transaction($this->_data['user_id'], 'subscribe_' . $node_type, $node_id, $price, 'out');
    /* subscribe to node */
    $this->subscribe($node_id, $node_type);
    $_SESSION['wallet_monetization_payment_amount'] = $price;
  }
   ...
   * @return string
   */
  public function coinpayments($handle, $price, $id = null, $node_type = null)
  {
    global $system;
    /* prepare */
    switch ($handle) {
      ...
      case 'subscribe':
        ...
        $URL['success'] = $system['system_url'] . "/settings/coinpayments";
        $URL['cancel'] = $system['system_url'];
        $URL['ipn'] = $system['system_url'] . "/includes/ajax/payments/coinpayments_webhook.php?handle=subscribe&node_id=$id&node_type=$node_type";
        break;
   ...
   * @return integer
   */
  public function fake_pages_generator($pages_num, $random_Avatar, $language, $category)
  {
    global $db, $system, $date;
    /* random Avatar */
    $random_Avatar = ($random_Avatar) ? true : false;
    ...
    while ($generated < $pages_num) {
      ...
      /* insert new page */
      $query = $db->query(sprintf("INSERT INTO pages (page_admin, page_category, page_name, page_title, page_description, page_picture, page_date) VALUES (%s, %s, %s, %s, %s, %s, %s)", secure($this->_data['user_id'], 'int'), secure($category, 'int'), secure($fake_username), secure($fake_title), secure($fake_title), secure($fake_avatar), secure($date)));
      if (!$query) continue;
      $generated++;
      /* get page_id */
      $page_id = $db->insert_id;
        ...
        switch ($field['type']) {
          ...
          case 'multipleselectbox':
            ...
            if (!is_array($value) || count($value) == 0) {
              throw new Exception(__("You must select at least one option from ") . " " . $field['label']);
            }
            break;
  ...
  public function get_reactions()
  {
    global $system, $db;
    $reactions = [];
    $get_reactions = $db->query("SELECT * FROM system_reactions") or _error("SQL_ERROR_THROWEN");
  ...
  public function send_invitation_email($email, $code)
  {
    ...
    $subject = html_entity_decode($this->_data['name'], ENT_QUOTES) . " " . __("Invite you to join") . " " . __($system['system_title']);
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'privacy':
        ...
        if (!in_array($args['user_privacy_poke'], $privacy) || !in_array($args['user_privacy_gifts'], $privacy) || !in_array($args['user_privacy_wall'], $privacy) || !in_array($args['user_privacy_gender'], $privacy) || !in_array($args['user_privacy_relationship'], $privacy) || !in_array($args['user_privacy_birthdate'], $privacy) || !in_array($args['user_privacy_basic'], $privacy) || !in_array($args['user_privacy_work'], $privacy) || !in_array($args['user_privacy_location'], $privacy) || !in_array($args['user_privacy_education'], $privacy) || !in_array($args['user_privacy_other'], $privacy) || !in_array($args['user_privacy_friends'], $privacy) || !in_array($args['user_privacy_followers'], $privacy) || !in_array($args['user_privacy_photos'], $privacy) || !in_array($args['user_privacy_pages'], $privacy) || !in_array($args['user_privacy_groups'], $privacy) || !in_array($args['user_privacy_events'], $privacy)) {
          _error(400);
        }
        /* update user */
        $db->query(sprintf(
      ...
      case 'monetization':
        ...
        /* check if price is valid */
        if ($args['user_monetization_enabled'] && (!is_numeric($args['user_monetization_price']) || $args['user_monetization_price'] <= 0)) {
          throw new Exception(__("You must enter valid monetization price"));
        }
        /* update user */
        $db->query(sprintf("UPDATE users SET user_monetization_enabled = %s, user_monetization_price = %s WHERE user_id = %s", secure($args['user_monetization_enabled']), secure($args['user_monetization_price'], 'float'), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
  ...
  public function sign_up($args = [])
  {
    ...
    if ($system['activation_enabled']) {
      if ($system['activation_type'] == "email") {
        ...
        $subject = __("Just one more step to get started on") . " " . __($system['system_title']);
    ...
    if ($user['user_two_factor_enabled']) {
      ...
      switch ($system['two_factor_type']) {
        case 'email':
          ...
          $method = __("Email");
          /* prepare activation email */
          $subject = __($system['system_title']) . " " . __("Two-Factor Authentication Token");
