...
try {
  ...
  switch ($_GET['do']) {
    ...
    case 'edit_extra':
      ...
      if (!isset($_POST['user_monetization_price'])) {
        $_POST['user_monetization_price'] = $user_info['user_monetization_price'];
      } else {
        if (is_empty($_POST['user_monetization_price']) || !is_numeric($_POST['user_monetization_price']) || $_POST['user_monetization_price'] <= 0) {
          throw new Exception(__("You must enter valid monetization price"));
        }
      }
      /* update */
      $db->query(sprintf("UPDATE users SET user_wallet_balance = %s, user_points = %s, user_monetization_enabled = %s, user_monetization_price = %s WHERE user_id = %s", secure($_POST['user_wallet_balance']), secure($_POST['user_points']), secure($_POST['user_monetization_enabled']), secure($_POST['user_monetization_price'], 'float'), secure($_GET['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
      /* return */
      return_json(array('success' => true, 'message' => __("User info have been updated")));
      break;
