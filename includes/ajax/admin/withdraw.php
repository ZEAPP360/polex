...
try {
  ...
  switch ($_POST['type']) {
    ...
    case 'monetization':
      /* get the request */
      $get_withdrawal_request = $db->query(sprintf("SELECT * FROM monetization_payments WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
      if ($get_withdrawal_request->num_rows == 0) {
        _error(400);
      }
      $withdrawal_request = $get_withdrawal_request->fetch_assoc();
      ...
      switch ($_POST['handle']) {
        case 'approve':
          /* approve request */
          $db->query(sprintf("UPDATE monetization_payments SET status = '1' WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
          /* send notification to request author */
          $user->post_notification(array('to_user_id' => $withdrawal_request['user_id'], 'action' => 'monetization_withdrawal_approved'));
          break;
        ...
        case 'decline':
          ...
          $db->query(sprintf("UPDATE monetization_payments SET status = '-1' WHERE payment_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
          /* update user balance */
          $db->query(sprintf("UPDATE users SET user_monetization_balance = user_monetization_balance + %s WHERE user_id = %s", secure($withdrawal_request['amount']), secure($withdrawal_request['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
          /* send notification to request author */
          $user->post_notification(array('to_user_id' => $withdrawal_request['user_id'], 'action' => 'monetization_withdrawal_declined'));
          break;
