...
try {
  ...
  switch ($_GET['do']) {
    ...
    case 'add':
      ...
      /* insert */
      $db->query(sprintf("INSERT INTO system_languages (system_languages.default, enabled, code, title, flag, dir, language_order) VALUES (%s, %s, %s, %s, %s, %s, %s)", secure($_POST['default']), secure($_POST['enabled']), secure($_POST['code']), secure($_POST['title']), secure($_POST['flag']), secure($_POST['dir']), secure($_POST['language_order'], 'int'))) or _error("SQL_ERROR_THROWEN");
      /* return */
