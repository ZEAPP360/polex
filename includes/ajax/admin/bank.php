  ...
  switch ($_POST['action']) {
    case 'accept':
      ...
      switch ($transfer_request['handle']) {
        ...
        case 'subscribe':
          /* check node */
          switch ($transfer_request['node_type']) {
            case 'user':
              // get user
              $node = $user->get_user($transfer_request['node_id']);
              if (!$node) {
                _error(400);
              }
              break;
            ...
            case 'page':
              // get page
              $node = $user->get_page($transfer_request['node_id']);
              if (!$node) {
                _error(400);
              }
              break;
            ...
            case 'group':
              // get group
              $node = $user->get_group($transfer_request['node_id']);
              if (!$node) {
                _error(400);
              }
              break;
          ...
          /* update user package */
          $user->subscribe($transfer_request['node_id'], $transfer_request['node_type'], $transfer_request['user_id']);
          break;
